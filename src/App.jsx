...
function Navbar({ totalCount, onCartToggle, onNavigateHome, onNavigateCategories, onNavigatePortfolio, currentPage, onSearch }) {
  const [searchValue, setSearchValue] = React.useState('')
  const handleSearch = () => {
    const q = searchValue.trim()
    if (q) { setSearchValue(''); onSearch(q) }
  }
  return (
    ...
    <footer className="bg-surface-container-lowest border-t border-outline-variant topographic-bg relative z-10">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter px-margin-mobile md:px-margin-desktop py-section-gap max-w-container-max mx-auto relative z-10">
        <div className="col-span-1 md:col-span-1">
          <img src="/storebass-blanco.png" alt="Storebass" className="h-32 w-auto mb-6" />
        </div>
...
function CheckoutPage({ cart, formatPrice, onNavigateHome, onPaymentProcessing }) {
  ...
  const handlePay = () => {
    const missingPersonal = !name || !lastName || !email || !phone
    const missingPayment = !selectedPayment
    const missingCard = selectedPayment === 'card' && (!cardNumber || !cardExp || !cardCvv || !cardHolder)
    const missingPolicy = !acceptPolicy
    if (missingPersonal || missingPayment || missingCard || missingPolicy) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <section className="bg-elevated-gray rounded-xl p-8 border border-card-border" data-purpose="payment-methods">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <input value={cardHolder} onChange={e => setCardHolder(e.target.value)} className="rounded-lg bg-transparent border border-white py-3 px-4 text-white placeholder-gray-400 focus:border-primary focus:outline-none" placeholder="Titular de la tarjeta" type="text" />
              </div>
            </div>
        ...
        <div className="lg:col-span-5">
          <aside className="bg-elevated-gray rounded-xl p-8 border border-card-border sticky top-24" data-purpose="order-summary">
            ...
            <button onClick={handlePay} className="w-full bg-transparent border-2 border-green-500 rounded-full py-4 px-6 flex items-center justify-between group hover:bg-green-500/10 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
          <div className="space-y-6">
            <div className="relative rounded-3xl overflow-hidden bg-elevated-gray aspect-[4/3] group">
              ...
              <button onClick={() => setFavorited(!favorited)} className={`absolute top-6 right-6 p-3 bg-black/40 backdrop-blur-md rounded-full transition-colors ${favorited ? 'text-red-500' : 'text-white hover:text-red-500'}`}>
                <svg className="w-6 h-6" fill={favorited ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"></path></svg>
              </button>
            </div>
...
function PaymentRejectedPage({ cart, formatPrice, onNavigateHome, onNavigateCheckout }) {
  const totalCount = cart.reduce((sum, item) => sum + item.quantity, 0)
  const subtotal = cart.reduce((sum, item) => sum + item.price * item.quantity, 0)
  const shipping = subtotal > 0 ? 8000 : 0
  const total = subtotal + shipping
  ...
  return (
    <main className="flex-grow w-full pt-28 pb-16">
      <div className="max-w-3xl mx-auto px-margin-mobile md:px-margin-desktop">
        <div className="bg-elevated-gray border border-card-border rounded-3xl overflow-hidden">
          <div className="bg-gradient-to-r from-red-600 to-red-500 p-10 text-center">
            <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-4">
          ...
          <div className="p-8 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-2">
              <button onClick={onNavigateCheckout} className="flex-1 bg-primary text-on-primary py-4 font-headline-lg text-xl uppercase rounded-xl hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2">
                <Icon name="edit" />
                Intentar de nuevo
              </button>
              <button onClick={onNavigateHome} className="flex-1 bg-transparent border-2 border-primary text-primary py-4 font-headline-lg text-xl uppercase rounded-xl hover:bg-primary hover:text-white transition-all flex items-center justify-center gap-2">
...
function SearchResultsPage({ query, products, onAddToCart, onNavigateProduct }) {
  return (
    <main className="flex-grow w-full pt-28 pb-16">
      <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
        <h2 className="font-headline-lg text-headline-lg uppercase border-l-4 border-primary pl-6 text-white mb-8">
            ...
            {products.map((product) => (
              <div key={product.id} onClick={() => onNavigateProduct?.(product.id)} className="product-card bg-elevated-gray border border-card-border rounded-xl overflow-hidden shadow-xl cursor-pointer" data-purpose="product-item">
                ...
                <div className="p-4 flex justify-between items-end">
                  ...
                  <button onClick={() => onAddToCart(product)} className="bg-primary text-on-primary p-2 rounded-lg hover:brightness-110 transition">
                    <span className="material-symbols-outlined text-sm">shopping_cart</span>
                  </button>
                </div>
              </div>
...
function PaymentProcessingPage({ success, onPaymentComplete, onPaymentRejected }) {
  React.useEffect(() => {
  ...
  }, [success, onPaymentComplete, onPaymentRejected])

  return (
    <main className="flex-grow w-full pt-28 pb-16">
      <div className="max-w-3xl mx-auto px-margin-mobile md:px-margin-desktop">
        <div className="bg-elevated-gray border border-card-border rounded-3xl overflow-hidden p-16">
...
function SplashPage({ onComplete }) {
  ...
  return (
    <main className="flex-grow w-full min-h-screen flex items-center justify-center bg-background">
      <div className="max-w-sm mx-auto px-margin-mobile w-full flex flex-col items-center gap-10">
        <img src="/texto-storebass.png" alt="Storebass" className="h-16 w-auto" />
        <div className="w-full flex flex-col items-center gap-4">
          <p className="text-white/60 text-sm uppercase tracking-widest font-label-sm">{label}</p>
          <div className="relative h-2 w-full overflow-hidden rounded-full bg-white/10">
            <div
              className="h-full rounded-full bg-gradient-to-r from-[#D4D400] via-[#E8E800] to-[#D4D400] relative"
              style={{ width: `${progress}%`, transition: 'width 0.3s ease' }}
            >
              <span className="absolute inset-y-0 left-0 w-1/2 rounded-full" style={{ background: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.55) 50%, transparent 100%)', mixBlendMode: 'screen', animation: 'sheen 1.6s linear infinite' }} />
            </div>
          </div>
...
function ProfileSelectorPage({ onSelectSebas, onSelectMap, onBack }) {
  const profiles = [
    {
      id: 'sebas',
      label: 'Sebas',
  ...
  return (
    <main className="flex-grow w-full min-h-screen flex items-center justify-center bg-background relative">
      <button onClick={onBack} className="absolute top-6 left-6 bg-transparent border-none cursor-pointer text-white/60 hover:text-primary transition-colors">
        <span className="material-symbols-outlined text-3xl">arrow_back</span>
      </button>
      <div className="flex flex-col items-center p-4">
        <h1 className="mb-10 text-3xl font-headline-lg text-white md:text-5xl uppercase text-center">
          Elige un perfil
        </h1>
        <div className="grid grid-cols-2 gap-8 md:gap-12">
          {profiles.map((profile) => (
            <div key={profile.id} className="flex flex-col items-center gap-4 group">
              <button
                onClick={() => profile.id === 'sebas' ? onSelectSebas() : onSelectMap()}
                aria-label={`Select profile: ${profile.label}`}
                className="relative h-28 w-28 md:h-36 md:w-36 rounded-full transition-transform duration-300 ease-in-out hover:-translate-y-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background"
              >
                <div className="absolute inset-0 rounded-full bg-surface-container transition-all duration-300 group-hover:shadow-lg group-hover:shadow-primary/20"></div>
              ...
              <p className="text-lg text-white/60 transition-colors group-hover:text-white font-title-md">
                {profile.label}
              </p>
            </div>
          ))}
...
export default function App() {
  ...
  const navigateFromLoading = useCallback(() => { setPage('home'); scrollTop() }, [scrollTop])
  ...
  const navigatePaymentRejected = useCallback(() => { setPage('paymentRejected'); scrollTop() }, [scrollTop])
  const navigatePaymentProcessing = useCallback((success) => { setPaymentSuccess(success); setPage('paymentProcessing'); scrollTop() }, [scrollTop])
  const navigateHome = useCallback(() => { setPage('home'); setScrollToCategory(null); setSelectedProduct(null); setSearchQuery(''); setPaymentSuccess(null); scrollTop() }, [scrollTop])
  const navigateCategories = useCallback((slug) => { setPage('categories'); setScrollToCategory(slug || null); setSelectedProduct(null) }, [])
  const navigateSearch = useCallback((query) => { setSearchQuery(query); setPage('search'); scrollTop() }, [scrollTop])
  const navigateProduct = useCallback((productId) => {
    const product = ALL_PRODUCTS.find(p => p.id === productId)
    if (product) { setSelectedProduct(product); setPage('product'); scrollTop() }
  }, [scrollTop])
  const navigatePortfolio = useCallback(() => { setPage('profileSelect'); scrollTop() }, [scrollTop])
  const navigatePortfolioSebas = useCallback(() => { setPage('portfolio'); scrollTop() }, [scrollTop])
  const navigatePortfolioMap = useCallback(() => { setPage('portfolioMap'); scrollTop() }, [scrollTop])

  return (
    <div className="bg-background text-white font-body-base antialiased min-h-screen">
      {page !== 'loading' && page !== 'profileSelect' && <Navbar totalCount={totalCount} onCartToggle={() => setIsOpen(true)} onNavigateHome={navigateHome} onNavigateCategories={navigateCategories} onNavigatePortfolio={navigatePortfolio} currentPage={page} onSearch={navigateSearch} />}

      {page === 'loading' ? (
        <SplashPage onComplete={navigateFromLoading} />
      ) : page === 'profileSelect' ? (
        <ProfileSelectorPage onSelectSebas={navigatePortfolioSebas} onSelectMap={navigatePortfolioMap} onBack={navigateHome} />
      ) : page === 'home' ? (
      ...
      ) : page === 'paymentRejected' ? (
        <PaymentRejectedPage cart={cart} formatPrice={formatPrice} onNavigateHome={navigateHome} onNavigateCheckout={navigateCheckout} />
      ) : page === 'paymentProcessing' ? (
        <PaymentProcessingPage success={paymentSuccess} onPaymentComplete={navigatePaymentComplete} onPaymentRejected={navigatePaymentRejected} />
      ) : page === 'portfolio' ? (
        <PortfolioPage />
      ) : page === 'portfolioMap' ? (
        <PortfolioPageMap />
      ) : page === 'search' && searchQuery ? (
        <SearchResultsPage query={searchQuery} products={ALL_PRODUCTS.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase()))} onAddToCart={addToCart} onNavigateProduct={navigateProduct} />
      ) : (
        <CheckoutPage cart={cart} formatPrice={formatPrice} onNavigateHome={navigateHome} onPaymentProcessing={navigatePaymentProcessing} />
      )}

      {page !== 'loading' && page !== 'profileSelect' && page !== 'portfolio' && page !== 'portfolioMap' && <FooterSection onNavigateCategories={navigateCategories} />}
